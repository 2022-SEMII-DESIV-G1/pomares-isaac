...
document.write( "<b>La suma del primer recorrrido es [" + Sumatot(sumar) + "]</b><br/>" );
  ...
  function calcular(Arreglo, i, j) {
    ...
    sumar = Arreglo[i][j] + 
        Math.max(
            calcular(Arreglo, i + 1, j), 
            calcular(Arreglo, i + 1, j + 1));
    return sumar;
                ...
                // Acceder al elemento
                  ...
                  document.write( "El valor de la posición [" + i + "," + j + "] es [" + Arreglo[i][j] + "]<br/>" );

                  contador1 = contador1 + Arreglo[i][j];      

                  for (j<-1;j<Arreglo.length[j];j++) {


                   }             
               }

          document.write( "<b>La suma del recorrido " +  j  + "  es ["  + contador1 + "]</b><br/>" );
